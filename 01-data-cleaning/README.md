...
- **Transaction Date** is stored inconsistently — some cells as plain text, others already as real date values. A combined formula `IFERROR(DATEVALUE(J2),IF(ISNUMBER(J2),J2,""))` handles both: converting text dates with `DATEVALUE` and passing real date values through directly.
...
![Data Quality Checklist](./screenshots/screenshots/data_quality_checklist.png)
...
![Raw Data](./screenshots/screenshots/raw_data.png)

---

### 3. Cleaned Data
Only the 7,579 rows that passed all checks. Pulled automatically from Raw Data
using INDEX/MATCH — no copy-paste. Uses TRIM-cleaned versions of all text
fields: Transaction ID, Customer ID, Category, Item, Payment Method, and Location.

![Cleaned Data](./screenshots/screenshots/cleaned_data.png)
...
### Step 1 — Apply TRIM to all text fields (columns L–R)
...
| R | Clean_Transaction_Date | `=IFERROR(DATEVALUE(J2),IF(ISNUMBER(J2),J2,""))` | Transaction Date (text → real date; handles both text and numeric date formats) |
...
### Step 2 — Flag every issue (columns S–AE)
...
### Step 3 — Count and label (columns AF, AG)
