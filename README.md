# excel-data-analyst-portfolio
Excel data analyst portfolio — 5 projects covering data cleaning, PivotTable dashboards, trend analysis, multi-source consolidation, and stakeholder reporting.
# 📊 Excel Data Analyst Portfolio

A portfolio of 5 Excel projects covering the full data analyst workflow — cleaning, dashboarding, trend analysis, multi-source consolidation, and stakeholder reporting.

## Projects

| # | Project | Skills |
|---|---------|--------|
| 1 | [Data Cleaning & Validation](./01-data-cleaning) | TRIM, PROPER, VLOOKUP, Data Validation, Conditional Formatting |
| 2 | [Pivot Dashboard](./02-pivot-dashboard) | PivotTables, PivotCharts, Slicers, KPI Cards |
| 3 | [Trend Analysis](./03-trend-analysis) | FORECAST.LINEAR, TREND, YoY %, STDEV.S |
| 4 | [Multi-Source Report](./04-multi-source-report) | Power Query, data consolidation |
| 5 | [Stakeholder Dashboard](./05-stakeholder-dashboard) | Requirements gathering, dashboard UX |

## Skills
`Advanced Formulas` `Power Query` `PivotTables` `Data Validation` `Dashboard Design` `Statistical Analysis`

## Contact
[LinkedIn] · [Email]
